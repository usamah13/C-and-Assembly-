...
uthread_mutex_t mx;
uthread_cond_t  canAdd, canRemove;
  ...
  for (int i=0; i<NUM_ITERATIONS; i++) {
    uthread_mutex_lock (mx);
      while (items == MAX_ITEMS)
        uthread_cond_wait (canAdd);
      items += 1;
      assert (items >= 0 && items <= MAX_ITEMS);
      histogram [items] ++;
      uthread_cond_signal (canRemove);
...
void* consumer (void* v) {
  for (int i=0; i<NUM_ITERATIONS; i++) {
    uthread_mutex_lock (mx);
      while (items == 0)
        uthread_cond_wait (canRemove);
      items -= 1;
      assert (items >= 0 && items <= MAX_ITEMS);
      histogram [items] ++;  
      uthread_cond_signal (canAdd);
    uthread_mutex_unlock (mx);
  }
  return NULL;
}
...
int main (int argc, char** argv) {
  ...
  canAdd    = uthread_cond_create (mx);
  canRemove = uthread_cond_create (mx);
