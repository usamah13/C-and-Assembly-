...
void list_map1 (void (*f) (element_t*, element_t), struct list* out_list, struct list* in_list) {
  ...
  for (int i = 0; i < list_len(in_list)-1; i++) {
    if (out_list->data[i] != NULL) {
     (*f) (out_list->data[i], list_get(in_list, i));  
     list_append (out_list, out_list->data[i]);
    } else {
      //out_list->data [i] = malloc(sizeof(element_t));
      (*f) (out_list->data[i], list_get(in_list, i));
      list_append (out_list, i, out_list->data[i]);
      //free (out_list->data [i]);
    }
  }
