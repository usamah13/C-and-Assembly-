...
void list_map1 (void (*f) (element_t*, element_t), struct list* out_list, struct list* in_list) {
  ...
  for (int i = 0; i < list_len(in_list); i++) {
     element_t final = NULL;
      f (&final, list_get(in_list, i));
      list_append (out_list, final);
  }
  // for (int i = 0; i < list_len(in_list)-1; i++) {
  //   if (out_list->data[i] != NULL) {
  //     element_t* final = out_list->data[i];
  //     f (final, list_get(in_list, i));
  //     list_append (out_list, *final);
  //     free (final);
  //   } else {
  //     element_t* final = malloc(sizeof(element_t));
  //     f (final, list_get(in_list, i));
  //     list_append (out_list, *final);
  //     free (final);
  //   }
...
void list_map2 (void (*f) (element_t*, element_t, element_t), struct list* out_list, struct list* in_list0, struct list* in_list1) {
  ...
  if (list_len(in_list0)<list_len(in_list1)) {
    min = list_len(in_list0);
  } else {
    min = list_len(in_list1);
  }
  for (int i = 0; i < min; i++) {
      element_t final = NULL;
      f (&final, list_get(in_list0, i), list_get(in_list1, i));
      list_append (out_list, final);
  }
}
